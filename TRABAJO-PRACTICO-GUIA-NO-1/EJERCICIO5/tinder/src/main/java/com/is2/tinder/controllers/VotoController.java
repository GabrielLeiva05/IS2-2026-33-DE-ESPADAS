...
public class VotoController {
        ...
        public String votar(HttpSession session, @RequestParam String idMascotaPropia,
                ...
                try {
                        votoService.votar(usuario.getId(), idMascotaPropia, idMascotaVotada);
                        attributes.addFlashAttribute("success", "Voto enviado con éxito");
        ...
        public ResponseEntity<String> descargarReporte(HttpSession session) {
                ...
                StringBuilder contenido = new StringBuilder(
                                "Nombre de Usuario;Apellido del Usuario;Nombre de la mascota;Cantidad de votos\n");
                votoService.listarReporteVotos()
                                .forEach(reporte -> contenido.append(reporte.getNombreUsuario()).append(';')
                                                .append(reporte.getApellidoUsuario()).append(';')
                                                .append(reporte.getNombreMascota()).append(';')
                                                .append(reporte.getCantidadVotos()).append('\n'));
                return ResponseEntity.ok()
                                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=reporte-votos.txt")
