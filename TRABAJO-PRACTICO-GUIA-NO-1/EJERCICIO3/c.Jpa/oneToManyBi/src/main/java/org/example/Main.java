        ...
        try {
            ...
            if (encontrada != null) {
                ...
                for (Domicilio d : encontrada.getDomicilios()) {
                    ...
                    System.out.println("Persona"+ d.getPersona().getNombre());
                }
            } else {
...
*/
            List<Domicilio> resultados = em.createQuery(
                            "SELECT d FROM Domicilio d WHERE d.ciudad = 'Mar del Plata'", Domicilio.class)
                    .getResultList();
            for(Domicilio dom : resultados){
